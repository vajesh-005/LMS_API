...
function Pending_requests() {
    ...
    const fetchAll = async () => {
      try {
        const [requests, names] = await Promise.all([
          fetch('http://localhost:2406/latestleaverequest/8'),
  ...
  const getDifference =(start,end )=>{
    const difference = Math.ceil((new Date(end)-new Date(start))/(1000*60*60*24))+1;
    return difference;
  }
      ...
      {pendingRequest?.map((item, index) => (
        <div key={index} className="pending-request-card">
          <div className="leave-type-and-status">
            <div className='leave-name'>{getLeaveName(item.leave_type_id)}</div>
            <div className={`status ${item.status === "Approved" ? "approved" : item.status === "Rejected" ? "rejected" : item.status==="Cancelled" ? "cancelled":"pending"}`}>
              {item.status}
            </div>
          ...
          <div className="date-and-days">
            <div className='date'> {item.start_date && new Date(item.start_date).toLocaleDateString()}
            </div>  
            <div className='date-difference'>
              {getDifference(item.start_date,item.end_date) + `${getDifference(item.start_date,item.end_date)>1 ? " Days" : " Day"}`}
         
            </div>
            <div className='date'>{item.end_date && new Date(item.end_date).toLocaleDateString()}
            </div>
          </div>
        </div>
      ))}

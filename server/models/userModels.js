...
exports.fetchTotalLeavesForUser = async (id) => {
    const query = 'SELECT user_id,total_remaining_days FROM remaining_leaves WHERE user_id = ? LIMIT 1';
...
exports.getPendingRequests = async (userId) => {
    const query = `SELECT * FROM leave_requests WHERE user_id = ? AND status = 'Pending'`;
